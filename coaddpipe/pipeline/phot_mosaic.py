...
def match_refcat(*args, **kwargs):
    ...
    if band == 'Y':
        ...
        ref = 'sdss'
        ref_band = band
    log("use {} band {}".format(ref, ref_band))

    icmd1 = 'select "{band}_{ref} < 90. && {band}_{ref} > 0' \
            ' && err_{band}_{ref} <= 0.10857"' \
        .format(band=ref_band, ref=ref)
    ...
    qa_catalog(cat_tbl, tbl, image_file, savename, ref, ref_band, kwargs)


def qa_catalog(cat_tbl, ref_tbl, image_file, savename, ref, ref_band, kwargs):
    log = get_log_func(default_level='debug', **kwargs)
    log('create QA plots')
    set_color()
    ...
    plot_zp_color(ax, ref_tbl, image_file, ref, ref_band)
...
def plot_zp_color(ax, ref_tbl, image_file, ref, ref_band):
    # band = re.match(r'.+_([ugrizY])\.fits$',
    #                 os.path.basename(image_file)).groups()[0]
    smag = '{}_{}'.format(ref_band, ref)
    semag = 'err_{}_{}'.format(ref_band, ref)
    smaglims = dict(
            z=(17, 19), u=(17, 19), y=(17, 19)).get(ref_band, (17, 20))
    mag = 'MAG_AUTO'
    emag = 'MAGERR_AUTO'
    sigma = 3.0
    ...
    k = hdr['MYCOLOR']
    c1, c2 = re.match(r"color term (\w)-(\w)",
                      hdr.comments['MYCOLOR']).groups()
    c1, c2 = '{}_{}'.format(c1, ref), '{}_{}'.format(c2, ref)
    # good mask
    cat = ref_tbl[
