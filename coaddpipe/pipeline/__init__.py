...
def get_tlist(config):
    ...
    t00 = dict(
        name='select images',
        func=prep_masking.select_images,
        pipe='transform',
    ...
    t41 = dict(
        ...
        params={'CATALOG_TYPE': 'ASCII_HEAD',
                ...
                'ANALYSIS_THRESH': 3,
                'DEBLEND_MINCONT': 0.05,
                'PHOT_APERTURES': [9, 14, 19, 24, 29, 34, 39, 44],
                # 'MAG_ZEROPOINT': 30,
    ...
    t520 = dict(
        ...
        follows=[t50, t51],
        after_func=phot_mosaic.header_to_config,
            )
    t52 = dict(
        name='create mosaic',
        func='swarp',
        ...
        in_=(t51, config['reg_grp']),
        add_inputs=[
            '{basename[0]}.fits', fmtname(config['fmt_mosaic_hdr']),
            '{imflag[0]}_{obsid[0]}'
            '_{object[0]}_{instru[0]}_{band[0]}.wht.fits',
            ],
        # =fmtname(config['fmt_mosaic_hdr']),
        in_keys=[('dummy', 'in', 'conf', 'WEIGHT_IMAGE'), ],
        out=[fmtname(config['fmt_mosaic']),
             fmtname(config['fmt_mosaic_wht'])],
        params={
            'HEADER_SUFFIX': '.{}'.format(config['phot_hdr_suffix']),
            'WEIGHT_TYPE': 'MAP_WEIGHT',
            'GAIN_KEYWORD': 'ARAWGAIN',
            # 'PIXELSCALE_TYPE': 'MANUAL',
            # 'PIXEL_SCALE': 0.20,
            'DELETE_TMPFILES': 'Y',
            'FSCALE_DEFAULT': '0/0',
