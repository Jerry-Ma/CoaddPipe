...
This script handles the photometry calibration of images.
...
chip by minimizing the differences.

The zero points of each chip consists of two components, one is
for the global transparency, which is uniform across the entire
image, the other is for per chip variation, which differs chip
by chip, but the intra-chip differences are the same for different
...
from ..apus.common import get_log_func
...
def main(*args, **kwargs):
    ...
    master.write("test_master.asc", format='ascii.commented_header')
    ...
    for i in catind:
        ...
        valid_s = []
        headers = []
        for ext, chip in layout.enumerate():
            extentry = master[
                    (master[ck['chip']] == chip) &
                    (master['catind'] == i)]
            print(extentry)
            if len(extentry) == 0:
                ...
                extentry = extentry[0]
                k = extentry['catkins']
                b = extentry['catbchip'] + extentry['catbair'] * airmass + \
                    extentry['catbcat']
                s = str(10 ** ((25.0 - b) / 2.5))
...
def match_refcat(*args, **kwargs):
    ...
    copycol = [
            ...
            ('y_{refkey}', 'REF_MAG_Y'), ('err_y_{refkey}', 'REF_ERR_Y'),
            ('ALPHA_J2000', 'OBS_RA'), ('DELTA_J2000', 'OBS_DEC'),
            ('MAG_AUTO', 'OBS_MAG_AUTO'), ('MAGERR_AUTO', 'OBS_ERR_AUTO'),
            ('XWIN_IMAGE', 'OBS_X'), ('YWIN_IMAGE', 'OBS_Y'),
            ]
    ...
    with fits.open(image_file, memmap=True) as hdulist:
        layout = get_layout(hdulist)
        # header columns
        for key in ['AIRMASS', 'EXPTIME']:
            col = Column([hdulist[0].header[key], ] * len(tbl), name=key)
            tbl.add_column(col)
        # get mjd time
        obstime = Time(
                hdulist[0].header['DATE-OBS'], format='isot', scale='utc')
        col_time = Column([obstime.mjd, ] * len(tbl), name='MJD')
        tbl.add_column(col_time)
    # odixy column
    chip = [layout.ext(ext)['chip'] for ext in tbl['EXT_NUMBER']]
    col_chip = Column(chip, name='CHIP')
    tbl.add_column(col_chip)
...
def get_regmask(xs, ys, exts, hdulist, regions, **kwargs):
    # log = get_log_func(default_level='debug', **kwargs)
    layout = get_layout(hdulist)
    filters = {chip: [] for chip in layout.chips()}
    for ext, chip, hdu in layout.enumerate(hdulist):
    ...
    for i in range(len(xs)):
        chip = layout.ext(exts[i])['chip']
        filter_list = filters[chip]
...
#     else:
#         raise ValueError('instru {0} not recognized'
...
def get_calibrated_master(in_cats, ck, model_flags, kwargs):
    ...
    master['catbchip'] = np.array([outpvals['b{0:s}'.format(o)]
                                  for o in master[ck['chip']]])
    master['catbchipunc'] = np.array([outpuncs['b{0:s}'.format(o)]
                                     for o in master[ck['chip']]])
...
def get_master_calib(tablelist, ck, kwargs):
    ...
    for i, fname in enumerate(tablelist):
        ...
        semag_lo, semag_up = ck['semaglims']
        log("quality cut {} < {} < {}; {} < err_{} < {}".format(
            smag_lo, ck['band'], smag_up,
            semag_lo, ck['band'], semag_up
            ))
        qualitymask = (smag_lo < tbl[ck['smag']]) & \
                      (tbl[ck['smag']] < smag_up) & \
                      (semag_lo < tbl[ck['semag']]) & \
                      (tbl[ck['semag']] < semag_up) & \
                      (zp > lolim) & (zp < uplim)  # & \
        tbl = tbl[qualitymask]
        # tbl = tbl[:200]str
        n = len(tbl)
        ...
        # (kins + kair * X) * (color) + bchip + bair * X + bcat
        for name in [
                'catkins', 'catkinsunc', 'catkair', 'catkairunc',
                'catbchip', 'catbchipunc', 'catbair', 'catbairunc',
...
def sc_func(params, color, zp, zperr, chip, catind, airmass):
    # (kins + kair * X) * (color) + bchip + bair * X + bcat
    bchip = np.array([params['b{0}'.format(o)].value for o in chip])
    bcat = np.array([params['bcat{0:.0f}'.format(o)].value for o in catind])
    model = (params['kins'].value + params['kair'].value * airmass) * color \
        + bchip + params['bair'].value * airmass + bcat
    return (model - zp) / zperr


def self_calibrate(bulk, ck, model_flags=('color', 'chip', 'expo', 'airmass')):
    '''
    For each exposure/chip, fit an offset so that the over all
    ...
    for c in np.unique(chip):
        vary = False if c == 'S1' else 'chip' in model_flags
        params.add('b{0}'.format(c), value=np.mean(zp1), vary=vary)
    for ind in np.unique(catind):
        vary = 'expo' in model_flags
    ...
    residue = sc_func(
            outparams, color, zp1, zp1err, chip, catind, airmass) * zp1err
...
def qa_master(master, ck, savename, kwargs):
    ...
    ax.set_ylabel(r'$m_{REF} - m_{ins} - k_{ins} \times (%s - %s) '
                  r'- b_{chip} - b_{X}X - b_{cat}$ (mag)' % cband)
...
def plot_zp_color(ax, bulk, ck, kwargs):
    ...
    for ii, i in enumerate(np.unique(catind)):
        ...
        # (kins + kair * X) * (color) + bchip + bair * X
        if np.any(im):
            ...
            shortname = re.match(r'.+_ct4m20(?:\d\d)(.+)_.+_\w\..+',
    ...
    # get pval bchip array
    bchip = np.ones((layout.NCY, layout.NCX), dtype='d') * np.nan
    bchipunc = np.ones(
            (layout.NCY, layout.NCX), dtype='d') * np.nan
    y0, x0 = layout.CY[0], layout.CX[0]
    for y in range(*layout.CY):
        for x in range(*layout.CX):
            c = layout.fxy(x, y, key='chip')
            # import pdb
            # pdb.set_trace()
            if c is None:
                continue
            if np.any(chip == c) > 0:
                bchip[y - y0, x - x0] \
                    = bulk['catbchip'][chip == c][0]
                bchipunc[y - y0, x - x0] \
                    = bulk['catbchipunc'][chip == c][0]
            else:
                log("no data found for CHIP {0} at fp {1}{2}".format(
                    c, x, y))

    bchip = bchip[::-1, :]
    pleg = px.imshow(
            bchip, interpolation='nearest', aspect=1, cmap=cm.coolwarm)
    cb = plt.colorbar(pleg, cax=cpx)
    cb.set_label('$b_{CHIP}$')
    px.yaxis.set_ticks(range(layout.NCY))
    px.yaxis.set_ticklabels(['Y{0}'.format(j) for j
                             in reversed(range(*layout.CY))])
    px.xaxis.set_ticks(range(layout.NCX))
    ...
    label = [
            r'$k_{ins}$=%.4f' % (kins),
            r'${\langle}b_{chip}{\rangle}$=%.4f' % (np.nanmean(bchip)),
            r'${\langle}{\delta}b_{chip}{\rangle}$=%.4f' % (
                np.nanmean(bchipunc)),
            r'${\Delta}b_{chip}$=%.4f' % (np.nanstd(bchip)),
            r'$b_{X}$=%.4f' % (bair),
            r'$n_{obj}$=%d' % (len(zp1)),
            r'${\Delta}res.$=%.5f' % (np.nanstd(res)),
            r'$n_{obj,clip}$=%d' % (len(zp1[clipmask])),
            r'${\Delta}res._{clip}$=%.5f' % (np.nanstd(res[clipmask]))
            ]
    ax.text(0.05, 0.92, '\n'.join(label),
