...
def main(args=None):
    ...
    from .utils import PathType, RecursiveHelpAction
    from . import core
    from .core import EXEC_NAME, DEFAULT_CONFIG_FILE
    from .version import version

    parser = argparse.ArgumentParser(
        add_help=False,
        description="""
A pipeline to co-add astronomical images (v{})""".format(version)
        )
    parser.add_argument(
    ...
    parser_run.add_argument(
            ...
            help="the ASCII table generated from the init step containing "
                 "info of the inputs for the pipeline",
            )
    parser_run.add_argument(
  ...
  $ {exec_name} setup               # setup the dir as {exec_name} workdir

  $ {exec_name} init testjob -i <paths to QR calibrated images>
  ...
  $ {exec_name} run testjob.txt ...
                                  # run the pipeline. The running of the
                                  # pipeline is powered by Apus,
                                  # run `{exec_name} run --apus-help` to
                                  # print a full help""".format(
                                      exec_name=EXEC_NAME)
                                  )  # noqa: W291
